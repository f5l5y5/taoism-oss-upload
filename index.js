#!/usr/bin/env node
// 获取脚本运行带入的参数 如node index.js --config alioss.config.js -flag true ====> { _: [ 1, 2, 3 ], config: 'alioss.config.js', flag: 'true' }
// -- 可以是后面完整的字符串
// 使用cross-env进行环境判断 不能用ENV的开头
var parseArgs = require('minimist')
const OSS = require('ali-oss')
// 直接使用node运行 不存在  使用yarn 可以获取 ？？？
const glob = require('glob')
const path = require('path')
const fs = require('fs')
const args = parseArgs(process.argv.slice(2))
// console.log('打印***process.env.PWD======>', path.resolve(__dirname, './dist/**/*'))
// console.log(process.env)
// console.log(glob.sync(path.resolve(__dirname, 'dist/**/*')).filter(v => fs.statSync(v).isFile())) // 找出所有的文件
// console.log(path.normalize('/dist/index.html')) // 找出所有的文件
// console.log(path.posix.normalize('/dist/index.html')) // 找出所有的文件
/**
 * @description 主函数
 */
async function main() {
	...
	// 获取配置文件
	const config = require(path.resolve(__dirname, args.config || 'alioss.config.js'))
	...
	const conf = {
		...
		accessKeyId: args.accessKeyId || accessKeyId,
		accessKeySecret: args.accessKeySecret || accessKeySecret,
...
async function upload(files, from, to, client) {
	return Promise.all(
		files.map(file => {
			...
			console.log('打印***targetPath======>', targetPath)
			return client
				.put(targetPath, file)
				.then(res => {
